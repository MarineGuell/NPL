...
--> fournit les outils de prétraitement et de traitement du texte

Inclut :
    Le nettoyage de texte (cleaning, clean_text)
    Le prétraitement (preprocess_text)
    La recherche Wikipedia (search_wikipedia)
    Le résumé de texte (summarize_text)
    La vectorisation de texte (text_to_tfidf, text_to_count, etc.)
    La gestion des expressions régulières
    Le formatage des formules mathématiques
